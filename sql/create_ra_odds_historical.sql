-- =============================================================================
-- CREATE CLEAN ra_odds_historical TABLE
-- =============================================================================
-- This script creates a fresh ra_odds_historical table from scratch
--
-- IMPORTANT: The old problematic table should be renamed to ra_odds_historical_backup
...
-- DROP TABLE IF EXISTS ra_odds_historical CASCADE;

-- STEP 2: Create the new clean table
CREATE TABLE ra_odds_historical (
...
CREATE INDEX idx_ra_odds_historical_date_of_race
ON ra_odds_historical (date_of_race);

-- Index for track-based queries
CREATE INDEX idx_ra_odds_historical_track
ON ra_odds_historical (track);

-- Index for horse name searches
CREATE INDEX idx_ra_odds_historical_horse_name
ON ra_odds_historical (horse_name);

-- Composite index for uniqueness detection
-- This helps identify duplicate records: same date, track, time, horse
CREATE INDEX idx_ra_odds_historical_unique_combo
ON ra_odds_historical (date_of_race, track, race_time, horse_name);

-- Index for data source filtering
CREATE INDEX idx_ra_odds_historical_data_source
ON ra_odds_historical (data_source);

-- Index for created_at (useful for incremental processing)
CREATE INDEX idx_ra_odds_historical_created_at
ON ra_odds_historical (created_at);

-- STEP 4: Add table comment
COMMENT ON TABLE ra_odds_historical IS
...
GRANT SELECT, INSERT, UPDATE, DELETE ON ra_odds_historical TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON ra_odds_historical TO service_role;
GRANT USAGE, SELECT ON SEQUENCE ra_odds_historical_racing_bet_data_id_seq TO authenticated;
GRANT USAGE, SELECT ON SEQUENCE ra_odds_historical_racing_bet_data_id_seq TO service_role;
...
--    ALTER TABLE ra_odds_historical RENAME TO ra_odds_historical_backup;
--
-- 2. Run this script in Supabase SQL Editor
--
-- 3. Verify creation with: SELECT COUNT(*) FROM ra_odds_historical;
--
-- 4. Test insertion with test_insertion.py
--
-- 5. If successful, optionally migrate data with migrate_from_backup.sql
--
-- 6. Keep ra_odds_historical_backup as safety until confirmed working
-- =============================================================================
